...
# State/federal race titles to exclude (exact match against parsed race_title)
STATE_FEDERAL_EXACT = {
...
# Filter: On Ballot only, exclude state/federal
local_on_ballot = [r for r in results if r['status_text'] == 'On Ballot' and r['race_title'] not in STATE_FEDERAL_EXACT]

print(json.dumps(local_on_ballot, indent=2))
print(f"\n\nTOTAL local on-ballot races: {len(local_on_ballot)}", file=sys.stderr)
